...
import com.devpath.career.application.SkillGapView;
...
@WebMvcTest(CareerReadinessController.class)
...
class CareerReadinessSecurityTest {
    private static final UUID USER_ID = UUID.fromString("e046a279-9c82-4bbf-9d8f-0737b222fa97");
    private static final UUID READINESS_ID = UUID.fromString("f823ffb9-88ea-4132-8388-9e5c7981912f");
    ...
    void allReadinessEndpointsRequireAuthenticatedOwnerContext() throws Exception {
        SkillGapView gap = new SkillGapView(UUID.randomUUID(), UUID.randomUUID(), "testing-discipline", "TESTING",
            new BigDecimal("50"), "DEVELOPING", new BigDecimal("60"), "PARTIAL",
            new BigDecimal("20"), List.of(UUID.randomUUID()));
        CareerReadinessView view = new CareerReadinessView(READINESS_ID, UUID.randomUUID(), "backend",
            UUID.randomUUID(), "career-v2", "readiness-v1", "baseline-v2", "COMPLETED",
            new BigDecimal("70"), "COMPETENT", new BigDecimal("85"), List.of(), List.of(gap),
            Instant.parse("2026-08-24T00:00:00Z"));
        when(service.getCurrent(USER_ID)).thenReturn(view);
        when(service.get(USER_ID, READINESS_ID)).thenReturn(view);
        when(service.getGaps(USER_ID, READINESS_ID)).thenReturn(new SkillGapListView(READINESS_ID, List.of(gap)));

        mockMvc.perform(get("/api/v1/career-readiness/current")).andExpect(status().isUnauthorized());
        mockMvc.perform(get("/api/v1/career-readiness/current").with(oauth2Login().oauth2User(principal())))
            .andExpect(status().isOk()).andExpect(jsonPath("$.data.readinessScore").value(70));
        mockMvc.perform(get("/api/v1/career-readiness/{id}", READINESS_ID)
                .with(oauth2Login().oauth2User(principal())))
            .andExpect(status().isOk()).andExpect(jsonPath("$.data.careerId").value("backend"));
        mockMvc.perform(get("/api/v1/career-readiness/{id}/skill-gaps", READINESS_ID)
                .with(oauth2Login().oauth2User(principal())))
            .andExpect(status().isOk()).andExpect(jsonPath("$.data.skillGaps[0].gapState").value("PARTIAL"));
        verify(service).getCurrent(USER_ID);
        verify(service).get(USER_ID, READINESS_ID);
        verify(service).getGaps(USER_ID, READINESS_ID);
    }
