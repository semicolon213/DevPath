...
public class AiGenerationApplicationService {
    public static final String TASK_TYPE = "SKILL_ANALYSIS_EXPLANATION";
    public static final String OUTPUT_TYPE = "SKILL_EXPLANATION";
    ...
    public AiGenerationApplicationService(
        ...
        SkillExplanationPromptBuilder prompts, SkillExplanationValidator validator, ObjectContentPort objects,
    ...
    public GenerationJobView request(
        ...
        UUID matrixId = sourceResourceRefs.getFirst();
        Optional<GenerationJob> repeated = persistence.findByOwnerAndIdempotencyKey(userId, idempotencyKey);
        if (repeated.isPresent()) {
            if (!persistence.findSkillMatrixId(repeated.get().promptContextId()).equals(matrixId)) {
                throw new IllegalArgumentException("Idempotency key belongs to another generation request");
            }
            return view(repeated.get());
        }
        var matrix = matrices.get(userId, matrixId);
        var template = persistence.loadActiveTemplate(TASK_TYPE);
        var prompt = prompts.build(matrix, template);
        Instant now = clock.instant();
        UUID contextId = UUID.randomUUID();
        persistence.saveContext(new StoredPromptContext(contextId, userId, template.id(), matrixId, TASK_TYPE,
            prompt.tokenBudget(), prompt.contextHash(), prompt.contextPayload(), prompt.prompt(), now));
        GenerationJob job = persistence.saveJob(GenerationJob.queue(userId, contextId, idempotencyKey, TASK_TYPE, now));
    ...
    public void providerSucceeded(
        ...
        var matrix = matrices.get(item.job().userId(), item.skillMatrixId());
        SkillExplanationValidation validation = validator.validate(result.content(), matrix);
        if (!validation.passed()) {
            boolean recorded = storeRejected(item, executionId, result.content(), validation.violations(), now);
        ...
        String content = json(validation.content());
        String reference = objects.put(current.userId(), artifactId, current.id(), "skill-explanation.json", content);
        try {
    ...
    public GeneratedArtifactView getArtifact(UUID userId, UUID artifactId) {
        ...
        SkillExplanationContent content = read(objects.read(userId, artifact.contentReference()));
    ...
    private GeneratedArtifactView artifactView(ArtifactRecord value, SkillExplanationContent content) {
        return new GeneratedArtifactView(value.artifactId(), value.artifactType(), "VALIDATED",
            new GeneratedArtifactView.Provenance(value.skillMatrixId(), value.promptContextId(),
    ...
    private String json(SkillExplanationContent value) {
    ...
    private SkillExplanationContent read(String value) {
        try { return mapper.readValue(value, SkillExplanationContent.class); }
    ...
    private static void validateRequest(
        ...
        if (!TASK_TYPE.equals(taskType) || !OUTPUT_TYPE.equals(outputType) || sourceResourceRefs == null
