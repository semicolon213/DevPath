...
public class LearningRoadmapController {
    ...
    @PostMapping("/{roadmapId}/archive") ApiResponse<LearningRoadmapView> archive(Authentication authentication,@PathVariable UUID roadmapId,@RequestHeader("Idempotency-Key") @Size(min=1,max=128) String idempotencyKey,@Valid @RequestBody ArchiveResourceRequest body,HttpServletRequest request){return ApiResponse.of(service.archiveRoadmap(userId(authentication),roadmapId),RequestIds.resolve(request));}
