...
| UserProfile | Stores the user's editable career presentation fields. | Maintain career stage and biography while using User display name as the canonical display name. | Profile identity, user identity, career stage, biography, timestamps, version. | Exactly one profile belongs to one User. | Created with account, updated, deleted with account. | Only the owning user may read or mutate it; display name is never duplicated. | User Aggregate | Profile Service | UserProfileUpdated |
| UserPreference | Preserves one career or company target selection event. | Record selected target and supersession history. | Preference identity, user identity, type, selected target slug, active flag, selected/superseded time, version. | Many historical preferences belong to one User; at most one active row per type. | Active, Superseded. | Targets must be active v1 catalog entries; extension candidates are rejected. | User Aggregate | Target Eligibility Service | TargetCareerChanged, TargetCompanyChanged |
