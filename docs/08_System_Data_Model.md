...
| UserProfile | Represents editable developer profile data. | Identity Context | Created with User and retained until account deletion/anonymization. | Career stage, biography, update version. | Internal profile identity and user identity. | Exactly one per User; User remains the source of truth for display name. |
| UserPreference | Represents a career or company target selection event. | Identity Context | Retained as active/superseded history until account deletion. | Target type, target slug, active state, selected/superseded timestamps. | Internal preference identity and user identity. | At most one active preference per user and target type. |
...
| CareerReadinessPolicy | Versioned deterministic readiness thresholds and career category weights. | Career Context | Immutable after activation. | Draft before activation. | Policy version, expected minimum, bands, weights, rounding, unsupported-input behavior. | Referenced by every CareerReadiness. |
...
| RecommendationPolicy | Immutable deterministic recommendation and career/category template version. | Recommendation Context | Published until superseded or deprecated. | Status only through version governance. | Priority mapping, type, prerequisite order, effort, completion criteria, expected evidence. | Produces RecommendationSet and binds RoadmapPolicy. |
